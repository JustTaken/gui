...
handle :: proc() -> bool {
  connection := wl.connect(context.allocator) or_return

  connection.display_id = wl.get_id(&connection, "wl_display", { wl.new_callback("error", error_callback), wl.new_callback("delete_id", null_callback) }, wl.WAYLAND_INTERFACES[:], context.allocator)
  connection.registry_id = wl.get_id(&connection, "wl_registry", { wl.new_callback("global", global_callback), wl.new_callback("global_remove", null_callback) }, wl.WAYLAND_INTERFACES[:], context.allocator)
...
global_callback :: proc(connection: ^wl.Connection, arguments: []wl.Argument) {
  ...
  case "wl_shm":
    connection.shm_id = wl.get_id( connection, "wl_shm", { wl.new_callback("format", format_callback) } , wl.WAYLAND_INTERFACES[:], context.allocator)
