...
init_wayland :: proc(arena: ^mem.Arena, tmp_arena: ^mem.Arena) -> bool {
  ...
  wl.connect(&connection, 600, 400, context.allocator) or_return

  connection.display_id = wl.get_id(&connection, "wl_display", { wl.new_callback("error", error_callback), wl.new_callback("delete_id", delete_callback) }, wl.WAYLAND_INTERFACES[:])
  connection.registry_id = wl.get_id(&connection, "wl_registry", { wl.new_callback("global", global_callback), wl.new_callback("global_remove", global_remove_callback) }, wl.WAYLAND_INTERFACES[:])
...
create_surface :: proc(connection: ^wl.Connection) {
  connection.surface_id = wl.get_id(connection, "wl_surface", { wl.new_callback("enter", enter_callback), wl.new_callback("leave", leave_callback), wl.new_callback("preferred_buffer_scale", preferred_buffer_scale_callback), wl.new_callback("preferred_buffer_transform", preferred_buffer_transform_callback) }, wl.WAYLAND_INTERFACES[:])
...
create_xdg_surface :: proc(connection: ^wl.Connection) {
  connection.xdg_surface_id = wl.get_id(connection, "xdg_surface", { wl.new_callback("configure", configure_callback) }, wl.XDG_INTERFACES[:])
...
create_shm_pool :: proc(connection: ^wl.Connection, max_width: u32, max_height: u32) {
  ...
  connection.shm_pool_id = wl.get_id(connection, "wl_shm_pool", {}, wl.WAYLAND_INTERFACES[:])
  connection.create_buffer_opcode = wl.get_request_opcode(connection, "create_buffer", connection.shm_pool_id)

  connection.buffer_id = wl.get_id(connection, "wl_buffer", { wl.new_callback("release", buffer_release_callback) }, wl.WAYLAND_INTERFACES[:])
...
global_callback :: proc(connection: ^wl.Connection, id: u32, arguments: []wl.Argument) {
  ...
  case "wl_shm":
    connection.shm_id = wl.get_id(connection, interface_name, { wl.new_callback("format", format_callback) } , wl.WAYLAND_INTERFACES[:])
  ...
  case "xdg_wm_base":
    connection.xdg_wm_base_id = wl.get_id(connection, interface_name, { wl.new_callback("ping", ping_callback) }, wl.XDG_INTERFACES[:])
  ...
  case "wl_compositor":
    connection.compositor_id = wl.get_id( connection, interface_name, { } , wl.WAYLAND_INTERFACES[:])
