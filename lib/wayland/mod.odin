...
wayland_init :: proc(
  ...
  ctx.ids.display = get_id(
    ...
    "wl_display",
    {
      new_callback("error", error_callback),
      new_callback("delete_id", delete_callback),
    },
    interface.WAYLAND_INTERFACES[:],
  ) or_return

  ctx.ids.registry = get_id(
    ctx,
    "wl_registry",
    {
  ...
  ctx.opcodes.get_registry = get_request_opcode(
    ctx,
    "get_registry",
    ctx.ids.display,
  ) or_return
  ...
  write(
    ctx,
    {interface.BoundNewId(ctx.ids.registry)},
    ctx.ids.display,
    ctx.opcodes.get_registry,
  ) or_return
...
surface_create :: proc(ctx: ^Wayland_Context) -> error.Error {
  ctx.ids.surface = get_id(
    ...
    "wl_surface",
    {
      ...
      new_callback(
        ...
        preferred_buffer_transform_callback,
      ),
    },
    interface.WAYLAND_INTERFACES[:],
  ) or_return

  ctx.opcodes.surface_attach = get_request_opcode(
    ctx,
    "attach",
    ctx.ids.surface,
  ) or_return

  ctx.opcodes.surface_commit = get_request_opcode(
    ctx,
    "commit",
    ctx.ids.surface,
  ) or_return

  ctx.opcodes.surface_damage = get_request_opcode(
    ctx,
    "damage",
    ctx.ids.surface,
  ) or_return
  ...
  write(
    ctx,
    {interface.BoundNewId(ctx.ids.surface)},
    ctx.ids.compositor,
    ctx.opcodes.create_surface,
  ) or_return
...
xdg_surface_create :: proc(ctx: ^Wayland_Context) -> error.Error {
  ...
  ctx.opcodes.get_toplevel = get_request_opcode(
    ctx,
    "get_toplevel",
    ctx.ids.xdg_surface,
  ) or_return

  ctx.opcodes.ack_configure = get_request_opcode(
    ctx,
    "ack_configure",
    ctx.ids.xdg_surface,
  ) or_return

  ctx.ids.xdg_toplevel = get_id(
    ctx,
    "xdg_toplevel",
    {
  ...
  write(
    ctx,
    {
      interface.BoundNewId(ctx.ids.xdg_surface),
      interface.Object(ctx.ids.surface),
    },
    ctx.ids.xdg_wm_base,
    ctx.opcodes.get_xdg_surface,
  ) or_return

  write(
    ctx,
    {interface.BoundNewId(ctx.ids.xdg_toplevel)},
    ctx.ids.xdg_surface,
    ctx.opcodes.get_toplevel,
  ) or_return
...
dma_create :: proc(ctx: ^Wayland_Context) -> error.Error {
  ctx.ids.dma_feedback = get_id(
    ...
    "zwp_linux_dmabuf_feedback_v1",
    {
      ...
      new_callback(
        ...
        dma_tranche_target_device_callback,
      ),
  ...
  ctx.opcodes.dma_feedback_destroy = get_request_opcode(
    ctx,
    "destroy",
    ctx.ids.dma_feedback,
  ) or_return

  write(
    ctx,
    {
      interface.BoundNewId(ctx.ids.dma_feedback),
      interface.Object(ctx.ids.surface),
    },
    ctx.ids.dma,
    ctx.opcodes.dma_surface_feedback,
  ) or_return
...
dma_params_init :: proc(ctx: ^Wayland_Context) -> error.Error {
  ...
  ctx.opcodes.dma_params_create_immed = get_request_opcode(
    ctx,
    "create_immed",
    ctx.ids.dma_params,
  ) or_return

  ctx.opcodes.dma_params_destroy = get_request_opcode(
    ctx,
    "destroy",
    ctx.ids.dma_params,
  ) or_return

  ctx.opcodes.dma_params_add = get_request_opcode(
    ctx,
    "add",
    ctx.ids.dma_params,
  ) or_return
...
write :: proc(
  ...
  for kind, i in request.arguments {
    ...
    case .BoundNewId:
      vector.write(
        interface.BoundNewId,
        &ctx.output,
        arguments[i].(interface.BoundNewId),
      ) or_return
    case .Uint:
      vector.write(
        interface.Uint,
        &ctx.output,
        arguments[i].(interface.Uint),
      ) or_return
    case .Int:
      vector.write(
        interface.Int,
        &ctx.output,
        arguments[i].(interface.Int),
      ) or_return
    case .Fixed:
      vector.write(
        interface.Fixed,
        &ctx.output,
        arguments[i].(interface.Fixed),
      ) or_return
    case .Object:
      vector.write(
        interface.Object,
        &ctx.output,
        arguments[i].(interface.Object),
      ) or_return
    case .UnBoundNewId:
      ...
      vector.padd_n(
        &ctx.output,
        u32(mem.align_formula(l, size_of(u32)) - l),
      ) or_return
    ...
    case .Fd:
      vector.append(&ctx.out_fds, arguments[i].(interface.Fd)) or_return
    case:
    }
  }
...
read :: proc(ctx: ^Wayland_Context) -> error.Error {
  ...
  for kind in event.arguments {
    #partial switch kind {
    case .Object:
      read_and_write(ctx, interface.Object) or_return
    case .Uint:
      read_and_write(ctx, interface.Uint) or_return
    case .Int:
      read_and_write(ctx, interface.Int) or_return
    case .Fixed:
      read_and_write(ctx, interface.Fixed) or_return
    case .BoundNewId:
      read_and_write(ctx, interface.BoundNewId) or_return
    case .String:
      read_and_write_collection(ctx, interface.String, &bytes_len) or_return
    case .Array:
      read_and_write_collection(ctx, interface.Array, &bytes_len) or_return
    case .Fd:
      read_fd(ctx) or_return
    case:
      return .OutOfBounds
    }
  }
...
read_fd :: proc(ctx: ^Wayland_Context) -> error.Error {
  vector.append(
    &ctx.values,
    vector.read(interface.Fd, &ctx.in_fds) or_return,
  ) or_return
  return nil
}

@(private)
read_and_write_collection :: proc(
  ctx: ^Wayland_Context,
  $T: typeid,
  length_ptr: ^u32,
) -> error.Error {
  ...
  bytes := vector.read_n(
    &ctx.input,
    u32(mem.align_formula(int(length), size_of(u32))),
  ) or_return
...
recv :: proc(ctx: ^Wayland_Context) -> error.Error {
  ...
  fds := (cast([^]interface.Fd)(raw_data(
        buf.data[alig:alig + size_of(interface.Fd) * fd_count],
      )))[0:fd_count]
...
send :: proc(ctx: ^Wayland_Context) -> error.Error {
  ...
  header := vector.buffer_new(
    u32(cmsg_align + mem.align_formula(int(fd_size), size_of(uint))),
    ctx.tmp_allocator,
  ) or_return
  ...
  socket_msg := posix.msghdr {
    ...
    msg_controllen = uint(
      mem.align_formula(int(header.vec.len), size_of(uint)),
    ),
    msg_flags      = {},
  }
...
object_append :: proc(
  ...
  if len(callbacks) != len(interface.events) {
    log.error(
      "Cannot create object interface if all events are not registred with callbacks",
    )
    return .OutOfBounds
  }
