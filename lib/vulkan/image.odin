...
image_create :: proc(
  ...
  aspect: Maybe(vk.ImageAspectFlags) = nil,
  ...
  image.memory = image_memory_create(
    ctx,
    image.handle,
    properties,
    memory_pNext,
...
image_from_file :: proc(
  ...
  if s, err := strings.split(path, ".", ctx.tmp_allocator); err == nil {
    if s[len(s) - 1] != "png" {
  ...
  if img, err := png.load_from_file(
    ...
    offset := staging_buffer_append(u8, ctx, img.pixels.buf[:]) or_return
...
//               fmt.printf(
//                 "[pHYs] Image resolution is %v x %v pixels per meter.\n",
//                 phys.ppu_x,
//                 phys.ppu_y,
//               )
//               fmt.printf(
//                 "[pHYs] Image resolution is %v x %v DPI.\n",
//                 dpi_x,
//                 dpi_y,
//               )
//               fmt.printf(
//                 "[pHYs] Image dimensions are %v x %v meters.\n",
//                 xm,
//                 ym,
//               )
//             } else {
//               fmt.printf(
//                 "[pHYs] x: %v, y: %v pixels per unknown unit.\n",
//                 phys.ppu_x,
//                 phys.ppu_y,
//               )
//             }
...
//   if err == nil &&
//      .do_not_decompress_image not_in options &&
//      .info not_in options {
...
//   if len(pix) == 0 ||
//      len(pix) < image.width * image.height * int(image.channels) {
//     return false
//   }
