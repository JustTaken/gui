...
@private
parse_scene :: proc(ctx: ^Context, raw: json.Object) -> (scene: Scene, err: error.Error) {
...
parse_scenes :: proc(ctx: ^Context) -> error.Error {
  for i in 0..<len(ctx.raw_scenes) {
    vector.append(&ctx.scenes, parse_scene(ctx, ctx.raw_scenes[i].(json.Object)) or_return) or_return
  }
