...
evaluate_node :: proc(ctx: ^Context, n: u32) {
  ...
  if ctx.nodes.data[n].parent != nil {
    ...
    ctx.nodes.data[n].transform.compose =
      ctx.nodes.data[ctx.nodes.data[n].parent.?].transform.compose *
      ctx.nodes.data[n].transform.compose
  }
  ...
  for i in 0 ..< ctx.animations.len {
    for k in 0 ..< ctx.animations.data[i].frames.len {
      ...
      transform.compose =
        ctx.nodes.data[n].transform.inverse * transform.compose

      if ctx.nodes.data[n].parent != nil {
        transform.compose =
          transforms.data[ctx.nodes.data[n].parent.?].compose *
          transform.compose
      }
    }
  }
