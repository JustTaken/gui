...
vulkan_buffer_create_memory :: proc(
	device: vk.Device,
...
wayland_buffer_write_swap :: proc(
	...
	write(
		ctx,
		{wl.Object(buffer.id), wl.Int(0), wl.Int(0)},
		ctx.surface_id,
		ctx.surface_attach_opcode,
	)
	write(
		ctx,
		{wl.Int(0), wl.Int(0), wl.Int(width), wl.Int(height)},
		ctx.surface_id,
...
wayland_buffer_create :: proc(ctx: ^WaylandContext, buffer: ^Buffer, width: u32, height: u32) {
	...
	for i in 0 ..< buffer.frame.modifier.drmFormatModifierPlaneCount {
		...
		write(
			ctx,
			{
				wl.Fd(buffer.frame.fd),
				wl.Uint(i),
				wl.Uint(plane.offset),
				wl.Uint(plane.rowPitch),
				wl.Uint(modifier_hi),
				wl.Uint(modifier_lo),
			},
			ctx.dma_params_id,
	...
	write(
		ctx,
		{wl.BoundNewId(buffer.id), wl.Int(width), wl.Int(height), wl.Uint(format), wl.Uint(0)},
		ctx.dma_params_id,
