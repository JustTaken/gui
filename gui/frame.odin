...
frames_init :: proc(ctx: ^Vulkan_Context, count: u32, width: u32, height: u32) -> Error {
	...
	for i in 0 ..< count {
		frame := &ctx.frames[i]
		frame.planes = alloc([]vk.SubresourceLayout, 3, ctx.allocator) or_return
...
create_frame :: proc(ctx: ^Vulkan_Context, frame: ^Frame, width: u32, height: u32) -> Error {
	...
	modifiers := alloc([]u64, u32(len(ctx.modifiers)), ctx.tmp_allocator) or_return
...
submit_staging_data :: proc(ctx: ^Vulkan_Context) -> Error {
	...
	update_descriptor_set(
		ctx,
		ctx.descriptor_sets[0],
		{ctx.uniform_buffer, ctx.model_buffer, ctx.color_buffer},
		{0, 1, 2},
...
frame_draw :: proc(ctx: ^Vulkan_Context, frame: ^Frame, width: u32, height: u32) -> Error {
	...
	for i in 0 ..< ctx.geometries_len {
		...
		vk.CmdBindIndexBuffer(cmd, geometry.indice, 0, .UINT16)
		vk.CmdDrawIndexed(
			cmd,
			geometry.count,
