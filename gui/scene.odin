...
tick_scene_animation :: proc(
  ...
  vulkan.update_transforms(
    &ctx.vk,
    vector.data(&on_going.ref.transforms.data[on_going.last_frame]),
    instance.scene.parent.transforms_offset,
  ) or_return
...
load_animations :: proc(
  ...
  animations = make(
    map[string]Scene_Animation,
    glt.animations.len * 2,
    ctx.allocator,
  )
  ...
  for i in 0 ..< glt.animations.len {
    ...
    animation.time_stamp = vector.new(
      f32,
      ref.frames.len,
      ctx.allocator,
    ) or_return

    animation.transforms = vector.new(
    ...
    for i in 0 ..< ref.frames.len {
      ...
      transforms := vector.new(
        Matrix,
        ref.frames.data[i].transforms.len,
        ctx.allocator,
      ) or_return
      ...
      for k in 0 ..< ref.frames.data[i].transforms.len {
        vector.append(
          &transforms,
          ref.frames.data[i].transforms.data[k].compose,
        ) or_return
      }
...
scene_instance_create :: proc(
  ctx: ^Context,
  scenes: ^Scenes,
  name: string,
  transform: Matrix,
  method: vulkan.Instance_Draw_Method,
) -> (
  instance: Scene_Instance,
  err: error.Error,
) {
...
scene_instance_update :: proc(
  ...
  for i in 0 ..< instance.models.len {
    for j in 0 ..< instance.models.data[i].instances.len {
      vulkan.instance_update(
        &ctx.vk,
        instance.models.data[i].instances.data[j],
        transform,
      ) or_return
    }
  }
...
model_instance_create :: proc(
  ...
  instance.children = vector.new(
    Model_Instance,
    instance.model.children.len,
    ctx.allocator,
  ) or_return
  ...
  for i in 0 ..< instance.model.children.len {
    vector.append(
      ...
      model_instance_create(
        ctx,
        scenes,
        instance.model.children.data[i],
        transform,
        method,
      ) or_return,
    ) or_return
  ...
  instance.instances = vector.new(
    ^vulkan.Instance,
    instance.model.geometries.len,
    ctx.allocator,
  ) or_return
  ...
  for i in 0 ..< instance.model.geometries.len {
    vector.append(
      &instance.instances,
      vulkan.geometry_instance_add(
        &ctx.vk,
        instance.model.geometries.data[i],
        transform,
        model,
        method,
      ) or_return,
...
load_unboned_mesh :: proc(
  ...
  for p in 0 ..< mesh.primitives.len {
    ...
    assert(
      size_of(Vertex) ==
      (size_of(f32) * positions.component_count) +
        (size_of(f32) * normals.component_count) +
        (size_of(f32) * textures.component_count),
    )
    ...
    geometry := vulkan.geometry_create(
      Vertex,
      &ctx.vk,
...
load_boned_mesh :: proc(
  ...
  for p in 0 ..< mesh.primitives.len {
    ...
    assert(
      size_of(Vertex) ==
      (size_of(u32) * joints.component_count) +
        (size_of(f32) * weights.component_count) +
        (size_of(f32) * positions.component_count) +
        (size_of(f32) * normals.component_count) +
        (size_of(f32) * textures.component_count),
    )
...
load_node :: proc(
  ...
  model.children = vector.new(
    u32,
    glt.nodes.data[node].children.len,
    ctx.allocator,
  ) or_return
  ...
  for i in 0 ..< glt.nodes.data[node].children.len {
    vector.append(
      &model.children,
      glt.nodes.data[node].children.data[i],
    ) or_return
  }
...
load_material :: proc(
  ...
  vector.append(
    &scenes.materials,
    vulkan.material_create(&ctx.vk, glt.materials.data[index].color) or_return,
  ) or_return
...
load_gltf_scene :: proc(
  ...
  scene.parent = scenes
  scene.models = vector.new(
    u32,
    glt.scenes.data[index].nodes.len,
    ctx.allocator,
  ) or_return
  ...
  for j in 0 ..< glt.scenes.data[index].nodes.len {
    vector.append(
      &scene.models,
      glt.scenes.data[index].nodes.data[j],
    ) or_return
  }
